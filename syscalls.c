...
asmlinkage long sys_plan9_rfork(struct pt_regs regs)
{
	long ret = -1;
	int clone_flags = 1;
	int return_value,ret1,ret2;
	unsigned long flags;
	unsigned long *addr = (unsigned long *)regs.sp;

	printk(KERN_INFO "In rfork system_call");
	printk(KERN_INFO "P9: Syscall %ld rfork called!\n", regs.ax);
	printk(KERN_INFO "P9: Syscall %ld rfork called!\n", regs.sp);
	
//	addr = addr+2;
	ret1 = get_user(flags, ++addr);
	printk(KERN_INFO "P9: rfork called with flag1%lx\n", flags);
	printk(KERN_INFO "P9: rfork called with return value ret1 %lx\n", ret1);
	
	addr = addr+2;
	ret2 = get_user(flags, addr);
	printk(KERN_INFO "P9: rfork called with flag2%lx\n", flags);
	printk(KERN_INFO "P9: rfork called with return value ret2 %lx\n", ret2);
	
	/*
	addr = addr+1;
	ret3 = get_user(flags, addr);
	printk(KERN_INFO "P9: rfork called with flag2%lx\n", flags);
	printk(KERN_INFO "P9: rfork called with return value ret2 %lx\n", ret3);
        */
	...
	if (flags & RFNAMEG) {
		printk(KERN_CRIT "rfork with RFCNAMEG called, unsharing!\n");
		return_value = sys_unshare(CLONE_NEWNS);
		printk(KERN_CRIT "return value of sys_unshare is: %d\n", return_value);
		//ret = sys_fork(regs);
		//printk(KERN_CRIT "return value of fork is: %d\n", ret);
	}
